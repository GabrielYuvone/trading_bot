...
def ejecutar_ciclo_bot():
    print(f"\n[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 🔄 Ejecutando ciclo de análisis...", flush=True)
    ...
    for symbol in SIMBOLOS:
        ...
        if posicion:
            side = posicion['side']
            print(f"📌 {symbol}: Posición abierta activa ({side.upper()}). Monitoreando salida...", flush=True)
            salir = False
            if side == 'long' and not st_actual:
                print(f"🚨 Señal de salida LONG en {symbol}: SuperTrend viró a bajista.", flush=True)
                salir = True
            elif side == 'short' and st_actual:
                print(f"🚨 Señal de salida SHORT en {symbol}: SuperTrend viró a alcista.", flush=True)
                salir = True
            ...
            if salir:
                try:
                    ...
                    print(f"✓ Posición cerrada en {symbol} por cambio de tendencia.", flush=True)
                except Exception as e:
        ...
        if signal:
            print(f"🎯 ¡Señal detectada para {symbol} ({signal.upper()})! ADX: {adx_actual:.2f}", flush=True)
            try:
                ...
                if signal == 'long':
                    ...
                    exchange.create_order(symbol, 'market', 'sell', amount, params={'stopPrice': sl_precio, 'triggerPrice': sl_precio})
                else:
                    exchange.create_market_order(symbol, 'sell', amount)
                    sl_precio = precio_mercado * (1 + PORCENTAJE_SL)
                    exchange.create_order(symbol, 'market', 'buy', amount, params={'stopPrice': sl_precio, 'triggerPrice': sl_precio})
                
                print(f"✓ Orden ejecutada para {symbol} [{signal.upper()}] | Tamaño: {amount} contratos | SL en {sl_precio:.4f}", flush=True)
            except Exception as e:
                print(f"❌ Error al abrir posición o SL en {symbol}: {e}", flush=True)

if __name__ == '__main__':
    print("🤖 INICIANDO BOT DE TRADING EN LA NUBE", flush=True)
    for symbol in SIMBOLOS:
