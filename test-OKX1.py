...
# Configuración de la API de OKX (Asegurate de cambiar a tus credenciales reales o testnet)
exchange = ccxt.okx({
    ...
    'timeout': 15000,
    'options': {
        'defaultType': 'swap'  # Modo futuros perpetuos / contratos por diferencia (USDT-margined)
    }
})

# Si usas Testnet de OKX, descomenta la siguiente línea:
exchange.set_sandbox_mode(True)

SIMBOLOS = ['BTC/USDT:USDT', 'ETH/USDT:USDT', 'SOL/USDT:USDT']
TIMEFRAME = '1h'
LEVERAGE = 50
CAPITAL_RIESGO_USDT = 50.0  # Margen base asignado por operación
...
def ejecutar_ciclo_bot():
    print(f"\n[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 🔄 Ejecutando ciclo de análisis...")
    ...
    for symbol in SIMBOLOS:
        ...
        fila_actual = df.iloc[-2]     # Vela cerrada anterior para evitar repintado
        ...
        if posicion:
            side = posicion['side']  # 'long' o 'short'
            print(f"📌 {symbol}: Posición abierta activa ({side.upper()}). Monitoreando salida...")
            
            # Condición de salida dinámica por tendencia
            salir = False
        ...
        if signal:
            print(f"🎯 ¡Señal detectada para {symbol} ({signal.upper()})! ADX: {adx_actual:.2f}")
            try:
                ticker = exchange.fetch_ticker(symbol)
                precio_mercado = ticker['last']
                
                # Obtener información del mercado para respetar el tamaño de contrato de OKX
                market = exchange.market(symbol)
                contract_size = market.get('contractSize', 1.0)
                
                # Calcular nocional y contratos reales
                ...
                print(f"✓ Orden ejecutada para {symbol} [{signal.upper()}] | Tamaño: {amount} contratos | SL en {sl_precio:.4f}")
            except Exception as e:
                print(f"❌ Error al abrir posición o SL en {symbol}: {e}")


if __name__ == '__main__':
    print("🤖 INICIANDO BOT DE PRODUCCIÓN / TESTNET (MULTISÍMBOLO)")
    for symbol in SIMBOLOS:
    ...
    while True:
        ...
        # Esperar 60 segundos antes de reintentar el chequeo del ciclo de 1 hora
        time.sleep(60)